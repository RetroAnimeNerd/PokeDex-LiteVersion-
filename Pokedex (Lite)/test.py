...
def comparePokemon(Pokemon1, Pokemon2):
    ...
    if f_id == s_id:
        results = f"These pokemon are fucking same do not piss me off!"
        return results
    else:
        if f_primaryType in type_disadvantage[s_primaryType]:
            typeAdvantageResults =  f"{f_name} is strong against {s_name}"
        if s_primaryType in type_disadvantage[f_primaryType]:
            typeAdvantageResults =  f"{s_name} is strong against {f_name}"
        
    return typeAdvantageResults
    
result = comparePokemon("Geodude", "Milotic")
print(result)
